...
tables[as.data.table(import("../../genomes/dm6/dmel-all-r6.36.gtf")), symbol:= gene_symbol, on= "FBgn==gene_id", mult="first"]
setcolorder(tables, 
            c("FBgn", 
              "symbol", 
...
tables[cl$regions, allograft_cl:= i.cl, on="FBgn"]
cl <- readRDS("Rdata/clustering_cutnrun_genotype_transcriptomes.rds")
tables[cl$regions, cutnrun_genotype_cl:= i.cl, on="FBgn"]
setcolorder(tables, c("FBgn", "symbol", "allograft_cl", "cutnrun_genotype_cl"))
