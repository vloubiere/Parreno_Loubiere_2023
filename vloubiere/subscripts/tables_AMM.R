...
tables <- rbindlist(lapply(setNames(tables, gsub("epiCancer_ED_|.txt|RNA_|system_","",basename(tables))), fread), 
                    idcol = T)
...
tables[cl$data, cutnrun_genotype_cl:= i.cl, on="FBgn"]
...
# Order and save
setcolorder(tables, 
            c("FBgn", "symbol", "cutnrun_genotype_cl", "PRC1_cluster"))
fwrite(tables, 
