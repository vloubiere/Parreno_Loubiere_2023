...
#------------------------------------#
# All REs
if(!file.exists("db/motif_enrichment_tables/dose_clusters_all.rds"))
{
  ...
  res <- vl_motif_cl_enrich(obj = mot, 
                            ...
                            bg = 0)
  saveRDS(res,
          "db/motif_enrichment_tables/dose_clusters_all.rds")
}

enr_all <- readRDS("db/motif_enrichment_tables/dose_clusters_all.rds")
enr_all <- enr_all[motif %in% enr_all[order(padj), motif[1], motif_name]$V1]

pdf("pdf/clustering/motif_enrichment_dose_clusters_all.pdf", 
    height = 12, 
    width = 7)
