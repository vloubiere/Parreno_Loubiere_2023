...
# Add tanscriptome padj
padj <- dcast(meta, FBgn~cdition, value.var = "padj")
dat <- merge(dat, 
             padj,
             by= "FBgn",
             suffixes= c("", "_padj"))
...
motif_enr <- vl_motif_cl_enrich(mot_counts,
                                cl_IDs = dat$cl,
                                plot= F)
motif_enr_cl2_vs_cl5 <- vl_motif_enrich(counts= mot_counts[dat$cl==2,],
                                        control_counts= mot_counts[dat$cl==5,],
                                        plot= F)
...
cl <- list(data= dat,
           ...
           mot_enrichment_cl2_vs_cl5= motif_enr_cl2_vs_cl5,
           GO= GO,
           network= net,
           som= som)
