...
meta[, sam_file:= paste0("/mnt/f/_R_data/projects/epigenetic_cancer/db/sam/ATAC/", gsub(".fastq.gz", ".sam", fq_basename)), fq_basename]
...
meta[, {
  ...
  if(!file.exists(peaks_file))
  {
    ...
    cmd <- paste0(cmd, " --keep-dup 1 -g dm --nomodel --outdir db/peaks/ATAC/ --name ")
...
if(!file.exists("db/peaks/ATAC/ATAC_merged_peaks.narrowPeak"))
{
  ...
  vl_exportBed(merged_peaks[N_replicates>=12, 1:10][order(seqnames, start)],
               "db/peaks/ATAC/ATAC_filtered_merged_peaks.narrowPeak")
}
